      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {services && services.map((plan: any, index: number) => {
              const packageKey = getPackageKey(plan.name, getServiceCategory());

              return (
                <Card
                  key={plan.name}
                  className={`relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-105 ${plan.is_featured
                  ...
                  {plan.is_featured && (
                    <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-blue-600 to-emerald-500 text-white text-center py-2">
                      <div className="flex items-center justify-center space-x-1">
                        ...
                        <span className="text-sm font-medium">Most Featured</span>
                        <Star className="h-4 w-4 fill-current" />
                      </div>
                    </div>
                  )}

                  <CardHeader className={`text-center ${plan.is_featured ? 'pt-12' : 'pt-6'}`}>
